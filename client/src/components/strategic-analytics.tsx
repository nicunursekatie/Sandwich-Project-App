...
export default function StrategicAnalytics() {
  ...
  const analyticsData = useMemo(() => {
    ...
    const totalSandwiches = collections.reduce(
      (sum, c) => sum + (c.individualSandwiches || 0) + calculateGroupTotal(c),
      0
    );
    ...
    const hostStats = collections.reduce(
      (acc, c) => {
        ...
        const sandwiches =
          (c.individualSandwiches || 0) + calculateGroupTotal(c);
      ...
      {} as Record<
        string,
        { total: number; collections: number; weeks: Set<string> }
      >
    );

    // Weekly analysis
    const weeklyData = collections.reduce(
      (acc, c) => {
    ...
    const weeklyTotals = Object.values(weeklyData)
      ...
      .sort((a, b) => b - a);
    const avgWeekly =
      weeklyTotals.reduce((sum, w) => sum + w, 0) / weeklyTotals.length || 0;
    ...
    const monthlyTrends = collections.reduce(
      ...
      {} as Record<
        string,
        { month: string; sandwiches: number; collections: number }
      >
    );
    ...
    const trendData: TrendData[] = Object.values(monthlyTrends)
      ...
      .map((m, i, arr) => ({
        ...
        trend:
          i > 0 && m.sandwiches > arr[i - 1].sandwiches
            ? 'growing'
            : m.sandwiches > avgWeekly * 4
              ? 'strong'
              : 'seasonal',
      }));
    ...
    const mostConsistent = Object.entries(hostStats).sort(
      ([, a], [, b]) => b.weeks.size - a.weeks.size
    )[0];

    const recordWeek = Object.entries(weeklyData).sort(
      ([, a], [, b]) => b.total - a.total
    )[0];
    ...
    const achievements: Achievement[] = [
      ...
      },
      {
        ...
        description: recordWeek
          ? `Outstanding performance on ${new Date(recordWeek[1].date).toLocaleDateString()}`
          : 'No data',
        icon: 'crown',
        highlight: true,
      },
      {
        ...
        description: topPerformer
          ? `${topPerformer[0]} leading with consistent excellence`
          : 'No data',
        icon: 'star',
        highlight: false,
      },
      ...
      },
      {
        ...
        description: mostConsistent
          ? `${mostConsistent[0]} with ${mostConsistent[1].weeks.size} weeks of participation`
          : 'No data',
        icon: 'star',
        highlight: false,
      },
    ...
    const seasonalInsights: SeasonalInsight[] = [
      {
        ...
        description:
          'September traditionally shows strong community engagement',
        opportunity:
          'Early fall presents excellent momentum-building opportunities',
        context:
          'Families returning from summer often seek ways to reconnect with community service',
      },
      {
        ...
        context:
          'Corporate giving programs and family traditions align with our mission',
      },
      {
        period: 'New Year Period',
        description: 'January offers fresh starts and renewed commitments',
        opportunity:
          'New Year resolutions create openings for expanded participation',
    ...
    const strategicOpportunities = [
      ...
      },
      {
        ...
        action:
          'Explore partnerships with local businesses for employee engagement',
      },
      {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {analyticsData.achievements.map((achievement, index) => (
              <Card
                key={index}
                className={
                  achievement.highlight ? 'border-primary shadow-md' : ''
                }
              >
        ...
        <TabsContent value="trends" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Understanding our momentum cycles
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-60">
