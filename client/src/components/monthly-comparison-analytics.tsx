...
export default function MonthlyComparisonAnalytics() {
  const [selectedTab, setSelectedTab] = useState<'overview' | 'hosts' | 'patterns' | 'insights'>('overview');
  const [compareYear, setCompareYear] = useState<number>(2025);

  // Fetch all collections data
  const { data: collectionsData, isLoading } = useQuery<{collections: SandwichCollection[]}>({
  ...
  const monthlyAnalytics = useMemo(() => {
    ...
    Object.values(monthlyStats).forEach(stats => {
      ...
      stats.avgPerCollection = stats.totalCollections > 0 ? Math.round(stats.totalSandwiches / stats.totalCollections) : 0;
  ...
  const augustAnalysis = useMemo(() => {
    ...
    const avgTopMonth = topMonths.reduce((sum, m) => sum + m.totalSandwiches, 0) / topMonths.length;
    ...
    return {
      ...
      shortfallPercent: ((avgTopMonth - august2025.totalSandwiches) / avgTopMonth * 100),
      yearOverYearChange: august2024 ? august2025.totalSandwiches - august2024.totalSandwiches : null,
      yearOverYearPercent: august2024 ? ((august2025.totalSandwiches - august2024.totalSandwiches) / august2024.totalSandwiches * 100) : null
    };
  }, [monthlyAnalytics]);
  ...
  const hostComparison = useMemo((): HostComparison[] => {
    ...
    Object.values(monthlyAnalytics).forEach(month => {
      Object.keys(month.hostParticipation).forEach(host => allHosts.add(host));
    });
    ...
    allHosts.forEach(hostName => {
      ...
      const otherMonths = Object.values(monthlyAnalytics)
        .filter(m => !(m.year === 2025 && m.month.includes('August')));
      ...
      const avgMonthlyTotal = monthlyTotals.reduce((sum, total) => sum + total, 0) / monthlyTotals.length;
      const difference = augustTotal - avgMonthlyTotal;
      const percentChange = avgMonthlyTotal > 0 ? (difference / avgMonthlyTotal * 100) : 0;

      // Count collections
      const augustCollections = collections.filter(c => 
        c.hostName === hostName && 
        c.collectionDate && 
      ...
      const avgMonthlyCollections = monthlyCollectionCounts.length > 0 
        ? monthlyCollectionCounts.reduce((sum, count) => sum + count, 0) / monthlyCollectionCounts.length 
        : 0;
  ...
  if (!augustAnalysis) {
    return (
      <div className="text-center py-16">
        ...
        <h3 className="text-xl font-semibold text-[#236383] mb-2">No August 2025 Data</h3>
        <p className="text-[#646464]">Unable to find August 2025 collection data for analysis.</p>
      </div>
    );
  }

  const colors = ['#236383', '#FBAD3F', '#3B82F6', '#10B981', '#F59E0B', '#EF4444'];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#236383]/10 to-[#FBAD3F]/10 p-6 rounded-lg border border-[#236383]/20">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-3xl font-bold text-[#236383] mb-2">August 2025 Performance Analysis</h2>
            <p className="text-[#646464]">Comprehensive analysis of August's lower collection performance</p>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border border-[#236383]/20">
            <div className="text-2xl font-bold text-[#236383]">
              {augustAnalysis.yearOverYearChange !== null ? 
                (augustAnalysis.yearOverYearChange > 0 ? '+' : '') + augustAnalysis.yearOverYearChange?.toLocaleString() :
                'N/A'
              }
            </div>
            <p className="text-sm text-[#646464]">vs August 2024</p>
            <div className={`text-sm mt-1 ${
              augustAnalysis.yearOverYearPercent && augustAnalysis.yearOverYearPercent > 0 ? 'text-green-600' : 'text-red-600'
            }`}>
              {augustAnalysis.yearOverYearPercent !== null ? 
                (augustAnalysis.yearOverYearPercent > 0 ? '+' : '') + augustAnalysis.yearOverYearPercent?.toFixed(1) + '%' :
                'First year'
              }
            </div>
          </div>
                  ...
                  <ComposedChart data={monthlyTrends}>
                    ...
                    <Tooltip 
                      formatter={(value, name) => [
                        ...
                        name === 'collections' ? 'Collections' : 'Hosts'
                      ]}
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-semibold text-[#236383] mb-3">Top 5 Months (2024-2025)</h4>
                  <div className="space-y-2">
                    {augustAnalysis.topMonths.slice(0, 5).map((month, index) => (
                ...
                <div>
                  <h4 className="font-semibold text-[#236383] mb-3">Performance Breakdown</h4>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm">Individual Collections</span>
                      <span className="font-medium">{augustAnalysis.august2025.individualCount.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm">Group Collections</span>
                      <span className="font-medium">{augustAnalysis.august2025.groupCount.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm">Average per Collection</span>
                      <span className="font-medium">{augustAnalysis.august2025.avgPerCollection}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm">Active Hosts</span>
                      <span className="font-medium">{augustAnalysis.august2025.uniqueHosts}</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#236383]/20">
                      <th className="text-left p-2 font-semibold text-[#236383]">Host</th>
                      <th className="text-right p-2 font-semibold text-[#236383]">Aug 2025</th>
                      <th className="text-right p-2 font-semibold text-[#236383]">Monthly Avg</th>
                      <th className="text-right p-2 font-semibold text-[#236383]">Difference</th>
                      <th className="text-right p-2 font-semibold text-[#236383]">% Change</th>
                      <th className="text-center p-2 font-semibold text-[#236383]">Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {hostComparison.slice(0, 15).map((host, index) => (
                      <tr key={host.hostName} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="p-2 font-medium" data-testid={`host-name-${index}`}>
                          {host.hostName.length > 25 ? host.hostName.substring(0, 25) + '...' : host.hostName}
                        </td>
                        <td className="p-2 text-right font-medium" data-testid={`host-august-${index}`}>
                          {host.augustTotal.toLocaleString()}
                        </td>
                        <td className="p-2 text-right" data-testid={`host-average-${index}`}>
                          {host.avgMonthlyTotal.toLocaleString()}
                        </td>
                        <td className={`p-2 text-right font-medium ${
                          host.difference >= 0 ? 'text-green-600' : 'text-red-600'
                        }`} data-testid={`host-difference-${index}`}>
                          {host.difference >= 0 ? '+' : ''}{host.difference.toLocaleString()}
                        </td>
                        <td className={`p-2 text-right ${
                          host.percentChange >= 0 ? 'text-green-600' : 'text-red-600'
                        }`} data-testid={`host-percent-${index}`}>
                          {host.percentChange >= 0 ? '+' : ''}{host.percentChange}%
                        </td>
                        <td className="p-2 text-center" data-testid={`host-status-${index}`}>
                          {host.percentChange >= 10 ? (
                            <Badge className="bg-green-100 text-green-700">Above Avg</Badge>
            ...
            <Card>
              ...
              <CardContent>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={[
                      { week: 'Week 1', sandwiches: augustAnalysis.august2025.weeklyDistribution[0] },
                      { week: 'Week 2', sandwiches: augustAnalysis.august2025.weeklyDistribution[1] },
                      { week: 'Week 3', sandwiches: augustAnalysis.august2025.weeklyDistribution[2] },
                      { week: 'Week 4+', sandwiches: augustAnalysis.august2025.weeklyDistribution[3] },
                    ]}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#236383" opacity={0.2} />
                      ...
                      <Tooltip 
                        ...
                        contentStyle={{ backgroundColor: 'white', border: '1px solid #236383', borderRadius: '8px' }}
                      />
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={[
                          { name: 'Individual', value: augustAnalysis.august2025.individualCount, color: '#236383' },
                          { name: 'Group', value: augustAnalysis.august2025.groupCount, color: '#FBAD3F' }
                        ]}
                        cx="50%"
                        cy="50%"
                        labelLine={false}
            ...
            <Card className="border-l-4 border-l-red-500">
              ...
              <CardContent className="space-y-3">
                <div className="p-3 bg-red-50 rounded">
                  <h4 className="font-semibold text-red-800 mb-1">Performance Gap</h4>
                  <p className="text-sm text-red-700">
                    August 2025 collected {augustAnalysis.shortfall?.toLocaleString()} fewer sandwiches 
                    ({augustAnalysis.shortfallPercent?.toFixed(1)}% below) compared to top performing months.
                  </p>
                </div>

                <div className="p-3 bg-amber-50 rounded">
                  <h4 className="font-semibold text-amber-800 mb-1">Host Participation</h4>
                  <p className="text-sm text-amber-700">
                    {hostComparison.filter(h => h.percentChange < -20).length} hosts performed significantly 
                    below their monthly average (&gt;20% decrease).
                  </p>
                </div>

                <div className="p-3 bg-blue-50 rounded">
                  <h4 className="font-semibold text-blue-800 mb-1">Seasonal Pattern</h4>
                  <p className="text-sm text-blue-700">
                    August consistently shows lower performance, suggesting seasonal factors 
                    (summer vacation, school schedules) impact collections.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-l-4 border-l-green-500">
              ...
              <CardContent className="space-y-3">
                <div className="p-3 bg-green-50 rounded">
                  <h4 className="font-semibold text-green-800 mb-1">August Planning</h4>
                  <p className="text-sm text-green-700">
                    Implement August-specific campaigns and host engagement programs 
                    to offset seasonal decline by June planning.
                  </p>
                </div>

                <div className="p-3 bg-purple-50 rounded">
                  <h4 className="font-semibold text-purple-800 mb-1">Host Support</h4>
                  <p className="text-sm text-purple-700">
                    Focus on supporting hosts who showed significant declines. Consider 
                    summer vacation scheduling and backup host arrangements.
                  </p>
                </div>

                <div className="p-3 bg-indigo-50 rounded">
                  <h4 className="font-semibold text-indigo-800 mb-1">Target Setting</h4>
                  <p className="text-sm text-indigo-700">
                    Set realistic August targets based on historical data while 
                    implementing specific strategies to minimize seasonal impact.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {hostComparison
                  ...
                  .slice(0, 6)
                  .map((host, index) => (
                    <div key={host.hostName} className="p-4 bg-green-50 rounded-lg border border-green-200">
                      <h4 className="font-semibold text-green-800 mb-2" data-testid={`success-host-${index}`}>
                        {host.hostName.length > 20 ? host.hostName.substring(0, 20) + '...' : host.hostName}
                      </h4>
                      <div className="text-2xl font-bold text-green-700" data-testid={`success-total-${index}`}>
                        {host.augustTotal.toLocaleString()}
                      </div>
                      <div className="text-sm text-green-600" data-testid={`success-change-${index}`}>
