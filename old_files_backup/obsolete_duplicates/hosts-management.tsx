...
export default function HostsManagement() {
  ...
  const [editingContact, setEditingContact] = useState<HostContact | null>(
    null
  );
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-2">
                    <User
                      className={`w-5 h-5 ${host.status === 'inactive' ? 'text-gray-400' : 'text-slate-600'}`}
                    />
                    <CardTitle
                      className={`text-base ${host.status === 'inactive' ? 'text-gray-600' : ''}`}
                    >
                      {host.name}
                    </CardTitle>
                  </div>
