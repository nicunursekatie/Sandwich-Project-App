...
import {
  ...
  insertSuggestionResponseSchema,
} from "@shared/schema";
...
router.get("/", isAuthenticated, async (req, res) => {
  try {
    ...
    if (
      user?.permissions?.includes(PERMISSIONS.SUGGESTIONS_VIEW) ||
...
router.post("/", isAuthenticated, async (req, res) => {
  try {
    ...
    try {
      ...
      if (emailSuccess) {
        console.log(
          "Developer notification email sent successfully for suggestion:",
          suggestion.id
        );
      } else {
        console.warn(
          "Failed to send developer notification email for suggestion:",
          suggestion.id
        );
      }
    } catch (emailError) {
...
router.post(
  "/:id/responses",
  requirePermission(PERMISSIONS.SUGGESTIONS_MANAGE),
  async (req, res) => {
    try {
      const suggestionId = parseInt(req.params.id);
